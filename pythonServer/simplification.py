...
def get_simplification(time_series,alpha):
    combined = [[ts,idx] for idx,ts in enumerate(time_series)]
    simp_ts = rdp(combined,epsilon=alpha)
    ys, xs = map(list, zip(*simp_ts))
    full_ts = interpolate_points_to_line(len(time_series), xs, ys)
