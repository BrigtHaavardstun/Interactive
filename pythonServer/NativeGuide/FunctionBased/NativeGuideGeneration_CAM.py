...
def native_guide_retrieval(query, predicted_label, distance, n_neighbors, data_set):
    ...
    y_pred = [np.argmax(predict_lite(data_set, X_train[idx])[0])
...
def load_weights(data_set):
    training_weights = np.load(
        'Class_Activation_Mapping/' + str(data_set) + '_cam_train_weights.npy')
    test_weights = np.load('Class_Activation_Mapping/' +
                           str(data_set) + '_cam_test_weights.npy')
    joint_weights = np.load('Class_Activation_Mapping/' +
                            str(data_set) + '_cam_joint_weights.npy')
...
def counterfactual_generator_swap(instance, nun_idx, subarray_length, data_set):
    ...
    most_influencial_array = findSubarray(
        (joint_weights[nun_idx]), subarray_length)

    starting_point = np.where(
        joint_weights[nun_idx] == most_influencial_array[0])[0][0]

    X_example = np.concatenate((instance.flatten()[:starting_point],
                                (X_train[nun_idx].flatten()[
...
def find_cf(instance, data_set):
    ...
    nun_idx = native_guide_retrieval(
        instance, pred_label, 'euclidean', 1, data_set)[0]
